...
def runs_scored(player_name: str) -> int:
    ...
    return deliveries.loc[
            deliveries.batter == player_name, 'batsman_runs'
            ].sum()
...
def highest_individual_score(player_name: str) -> int:
    ...
    return batsman_scores.loc[
            batsman_scores.batter == player_name, 'batsman_runs'
            ].max()
...
def total_50s(player_name: str) -> int:
    ...
    '''
    return batsman_scores.loc[
            (batsman_scores.batter == player_name) & (batsman_scores.batsman_runs >= 50) & 
            (batsman_scores.batsman_runs < 100), 'batsman_runs'
            ].count()
...
def potm_awards(player_name: str) -> int:
    ...
    '''
    return matches.loc[
            matches.player_of_match == player_name
...
def batting_average(player_name: str) -> float:   
    ...
    '''
    number_of_dismissals = deliveries[
...
def strike_rate(player_name: str) -> float:   
    ...
    '''
    return round(
            (runs_scored(player_name)/
            deliveries[deliveries.batter == player_name].shape[0])*100,2
            )
...
def favorite_teams_to_score(player_name: str) -> pd.DataFrame:
    ...
    '''
    batsman_scores_2 = deliveries.groupby(
            ['batter', 'bowling_team']
            )['batsman_runs'].sum().reset_index()
    
    favorites = batsman_scores_2[
                batsman_scores_2.batter == player_name
                ].sort_values(
                    by = 'batsman_runs', 
                    ascending=False
...
def favorite_team_for_bowlers(player_name: str) -> pd.DataFrame:
    ...
    '''
    bowling_teams = deliveries[
                    deliveries.is_wicket == 1
                    ].groupby(
                        ['bowler', 'batting_team']
                    )['is_wicket'].count().reset_index()
    
    favorites = bowling_teams.loc[
                bowling_teams.bowler == player_name
                ].sort_values(
                    by = 'is_wicket', 
                    ascending=False
...
def most_successful_venue(player_name: str) -> tuple:
    ...
    # for runs
    runs_by_venue = deliveries_with_venue.groupby(
                    ['batter','venue']
                    )['batsman_runs'].sum().reset_index()
    
    best_venue_for_runs = runs_by_venue.loc[
                        runs_by_venue.batter == player_name
                        ].sort_values(
                            by='batsman_runs', 
                            ascending=False
                        ).head(1)['venue']
    ...
    # for wickets
    wickets_by_venue = deliveries_with_venue.groupby(
                        ['bowler','venue']
                        )['is_wicket'].count().reset_index()
    
    best_venue_for_wickets = wickets_by_venue.loc[
                                wickets_by_venue.bowler == player_name
                                ].sort_values(
                                    by='is_wicket', 
                                    ascending=False
                                ).head(1)['venue']
